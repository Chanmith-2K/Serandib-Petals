...
function selectedReportOrders(){
 const all=getOrders();const month=document.getElementById("reportMonth")?.value||"";
 if(reportMode==="all"||!month)return all;
 return all.filter(o=>(o.deliveryDate||"").slice(0,7)===month);
}
function renderAnalytics(){
 const monthEl=document.getElementById("reportMonth");if(monthEl&&monthEl.value)reportMode="month";
 const orders=selectedReportOrders(),revenue=orders.reduce((s,o)=>s+Number(o.amount||0),0),paid=orders.filter(o=>o.paymentStatus==="Paid").length,open=orders.filter(o=>o.status!=="Delivered").length,del=orders.filter(o=>o.status==="Delivered").length;
...
function exportCSV(){
 const orders=selectedReportOrders();const headers=["Order ID","Customer","Phone","Bouquet","Flower Details","Card Message","Delivery Date","Delivery Address","Amount LKR","Payment Status","Order Status","Notes"];
...
function backupJSON(){downloadBlob(JSON.stringify({version:2,exportedAt:new Date().toISOString(),orders:getOrders()},null,2),"serendib-orders-backup-"+new Date().toISOString().slice(0,10)+".json","application/json")}
...
function resetAllData(){
 if(!confirm("This will delete ALL orders stored on this device. Continue?"))return;const code=prompt('Type RESET to permanently delete all orders on this device.');if(code!=="RESET"){alert("Reset cancelled.");return}setOrders([]);renderOrders();renderAnalytics();alert("All orders on this device have been reset.")
}
...
async function shareSummary(){
 const orders=selectedReportOrders(),rev=orders.reduce((s,o)=>s+Number(o.amount||0),0),period=reportMode==="all"?"All Time":monthLabel(document.getElementById("reportMonth")?.value||"");
